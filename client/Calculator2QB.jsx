...
Results2QB = React.createClass({
  renderTeam1 () {
    return this.props.team2.map((player) => {
      return (
          <tr>
            <td><Link key={player._id} to={"/2qb/" + player.id}>{player.name}</Link></td>
            <td>{player.value}</td>
          </tr>
        )
    });
    ...
    return (
        <div>
          ...
          <div className="row">
            ...
            <div className="col-xs-12 text-center">
              <h3>The difference is equivalent to <Link key={closestPlayer._id} to={"/2qb/" + closestPlayer.id}>{closestPlayer.name}</Link> with an ADP of {closestPlayer.jun_16}</h3>
            </div>
...
Calculator2QB = React.createClass({
  ...
  getMeteorData() {
    return {
      players: Players2QB.find({}, {sort: {jun_16: 1}}).fetch()
    };
  },
